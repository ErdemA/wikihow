...
class DumpStuData {
	...
	function __construct() {
		...
		$this->dbw = new Database(WH_DATABASE_BACKUP, WH_DATABASE_MAINTENANCE_USER, WH_DATABASE_MAINTENANCE_PASSWORD, self::STU_DATABASE);
	...
	function getAllPages() {
		...
		foreach($domains as $domain) {
			...
			foreach($pageTitles as $pageTitle) {
				$pages[] = $pageTitle;
				if(sizeof($pages) >= 1000) {
					$this->getPagesWithRetry($pages, $domain);
					$pages = array();
				}
			}
			$this->getPagesWithRetry($pages, $domain);
