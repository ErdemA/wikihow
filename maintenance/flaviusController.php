...
class FlaviusController {
	 ...
	 */
	public function setErrorEmails($emails) {
		$this->errorEmails[] = $emails;	
	}
	 ...
	 */
	public function calcIntervalStat($statName, $lastTouchDate=false, $partial=false) {
		if($lastTouchdate) {
			$ids = $this->flavius->getIdsToCalc($lastTouchDate);
		}
		else {
		...
		$this->doBatch($ids,self::BATCH_SIZE,function($idSlice) use($stats,$t) {
			$t->flavius->calcIntervalStats($idSlice, $stats, $t->oldDay, $t->yesterdaysDate);
...
$fc->setErrorEmails(array("gershon@wikihow.com"));
