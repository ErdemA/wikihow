...
// explicitly register the mediawiki __autoload function since composer uses spl_autoload_register
...
class WikiVideoProcess {
	...
	*/
	 ...
	 */
	private static function dbSetArticleProcessed($articleID, $creator, $error, $warning, $url, $numVids, $numSteps, $replaced, $status) {
...
//}
		...
		foreach ($articles as $id => $details) {
			...
			*/
			...
			if (!$err) {
				...
				self::transcodeVideos($id, $details['user'], $videoList, $warning);
			} else {
				self::dbSetArticleProcessed($id, $details['user'], $err, '', '', 0, 0, 0, self::STATUS_ERROR);
			}
	...
	private static function transcodeVideos($aid, $creator, &$videoList, $warning) {
		$err = '';
		$s3 = new S3(WH_AWS_WIKIVIDEO_PROD_ACCESS_KEY, WH_AWS_WIKIVIDEO_PROD_SECRET_KEY);
		...
		if (!$err) {
			...
			if ($err) {
				self::dbSetArticleProcessed($aid, $creator, $err, $warning, $url, 0, $numSteps, 0, self::STATUS_ERROR);
				...
				self::dbSetArticleProcessed($aid, $creator, '', $warning, $url, count($videoList), 0, 0, self::STATUS_TRANSCODING);
			}
		}
	}
	...
	*/
	...
	public static function main() {
		...
		self::loginAsUser(self::VIDEO_USER);
		if ($doCleanup) {
			self::doS3Cleanup();
		} else {
