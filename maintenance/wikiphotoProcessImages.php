...
class WikiPhotoProcess {
	...
	*/
	 ...
	 */
	private static function placeImagesInSteps($articleID, $title, &$images, &$text, &$stepsText, &$numSteps, &$replaced) {
		$errs = '';
	...
	private static function processImages($articleID, $creator, $imageList, $warning) {
		...
		if (!$err) {
			$err = self::placeImagesInSteps($articleID, $title, $imageList, $text, $steps, $numSteps, $replaced);
		}
