...
class LoginForm {
	...
	function LoginForm( &$request, $par = '' ) {
		...
		if( $this->mReturnTo == $wgLang->specialPage( 'Userlogout' ) ) $this->mReturnTo = '';
		if( $this->mAutoRedirect == $wgLang->specialPage( 'Userlogout' ) ) $this->mAutoRedirect = '';
	}
	...
	function addNewAccount() {
		...
		if( $wgUser->isAnon() ) {
			...
			if( $this->hasSessionCookie() ) {
				...
				$redirectURL = wfMsg('welcomecreation-landing');
				wfRunHooks('NewUserRedirect', array(&$redirectURL));
				return $this->successfulLogin( $redirectURL, false, true );
			} else {
...
"); 
	...
	function mainLoginForm( $msg, $msgtype = 'error' ) {
		...
		global $wgAuth, $wgEmailConfirmToEdit, $wgSSLsite;
		...
		if (SSL_LOGIN_DOMAIN && !$wgSSLsite) {
			header('Location: https://' . SSL_LOGIN_DOMAIN . @$_SERVER['REQUEST_URI']);
			exit;
		}
		...
		if( !empty( $this->mAutoRedirect ) ) {
			$autoredirect = '&autoredirect=' . wfUrlencode( $this->mAutoRedirect );
			$q .= $autoredirect;
		}
		...
		$template->set( 'userealname', false );
	...
	function topContent($form){
		if($form->mType == "signup"){
			echo '<p class="wh_block"><span class="login_top_link">' . $form->mLink . '</span><span class="login_headline">' . wfMsg('createaccount') . '</span></p>';
		}
		else if($form->mMailmypassword){
			echo '<p class="login_header wh_block"><span class="login_top_link">' . $form->mLink . '</span><span class="login_headline">' . wfMsg('accmailtitle') . '</span> </p>';
		}
		else{
			echo '<p class="wh_block"><span class="login_top_link">' . $form->mLink . '</span><span class="login_headline">' . wfMsg('login') . '</span> </p>';
		}
		return true;
	}
