	...
	*/
	function getContent() {
		...
		if ( 0 == $this->getID() ) {
			...
			$ret =  "<div class='noarticletext'>$ret</div>";
			if ($this->mTitle->getNamespace() != NS_MEDIAWIKI) {
				$ret = "<h2>" . wfMsg('noarticlefound') . "</h2>$ret";
			}
			return $ret;
		} else {
			$this->loadContent();
	...
	function fetchContent( $oldid = 0 ) {
		...
		if( $oldid ) {
			...
			if( is_null( $revision ) ) {
				// XXCHANGED -- make http response code 404 for this error
				global $wgOut;
				if ($wgOut) $wgOut->setStatusCode(404);
...
#echo wfBacktrace(); exit;
	...
	function delete() {
		...
		if( $hasHistory && !$confirm ) {
			$skin=$wgUser->getSkin();
			$wgOut->addHTML( '<h3>' . wfMsg( 'historywarning' ) . '</h3>' . $skin->historyLink() . '' );
			if( $bigHistory ) {
				global $wgLang, $wgDeleteRevisionsLimit;
				$wgOut->wrapWikiMsg( "<div class='error'>\n$1</div>\n",
	...
	function confirmDelete( $par, $reason ) {
		...
		$form = Xml::openElement( 'form', array( 'method' => 'post', 'action' => $this->mTitle->getLocalURL( 'action=delete' . $par ), 'id' => 'deleteconfirm' ) ) .
			...
			"<tr id=\"wpDeleteReasonListRow\">
				...
				<td>" .
					Xml::listDropDown( 'wpDeleteReasonList',
						...
						wfMsgForContent( 'deletereasonotherlist' ), $defaultReason, 'wpReasonDropDown input_med', 1 ) .
				"</td>
			</tr>
			<tr id=\"wpDeleteReasonRow\">
				...
				"</td>
				<td>" .
					Xml::input( 'wpReason', 60, $reason, array( 'type' => 'text', 'maxlength' => '255', 'tabindex' => '2', 'id' => 'wpReason', 'class' => 'input_med') ) .
				"</td>
			</tr>
			<tr>
				<td></td>
			...
			<tr>
				<td></td>
				<td>" .
					Xml::submitButton( wfMsg( 'deletepage' ), array( 'name' => 'wpConfirmB', 'id' => 'wpConfirmB', 'tabindex' => '4', 'class' => 'button primary') ) .
				"</td>
			</tr>" .
