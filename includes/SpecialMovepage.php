...
class MovePageForm {
	...
	function showForm( $err, $hookErr = '' ) {
		...
		$encOldTitle = '';
		if ((!$ot || !$ot->exists()) && preg_match('@^([0-9]+)$@', $this->oldTitle)) {
			$id = intval($this->oldTitle);
			$ot = Title::newFromID( $id );
			if ($ot) {
				$this->oldTitle = $ot->getPartialURL();
				$encOldTitle = $id;
			}
		}
		...
		if (!$encOldTitle) $encOldTitle = htmlspecialchars( $oldTitle );
		if( $this->newTitle == '' ) {
		...
		if ( $err == 'articleexists' && $wgUser->isAllowed( 'delete' ) ) {
			...
			$wgOut->addWikiMsg( 'movepagetext_owl' );
...
<form id=\"movepage\" method=\"post\" action=\"{$action}\">
	<table border='0'>
		<tr>
			<td align='$end' class='move_article_label' valign='middle'>{$movearticle}</td>
			<td align='$start'>{$oldTitleLink}</td>
		</tr>
		<tr>
			<td align='$end' valign='middle'><label for='wpNewTitle'>{$newtitle}</label></td>
			<td align='$start'>
				<input class='input_med' type='text' size='40' name='wpNewTitle' id='wpNewTitle' value=\"{$encNewTitle}\" />
				<input class='input_med' type='hidden' name=\"wpOldTitle\" value=\"{$encOldTitle}\" />
			</td>
		</tr>
		<tr>
			<td class='input_med' align='$end' valign='middle'><label for='wpReason'>{$movereason}</label></td>
			<td align='$start' valign='middle'>
				<textarea class='input_med' cols='40' rows='3' name='wpReason' id='wpReason'>{$encReason}</textarea>
			</td>
		</tr>" );
		...
		<tr>
			<td>&nbsp;</td>
			<td align='$start'>
				<input type='submit' class='button primary' name=\"{$submitVar}\" value=\"{$movepagebtn}\" />
			</td>
		</tr>
	</table>
	...
	function doSubmit() {
		...
		// XXCHANGED for wikiHow if newTitle contains dashes, substitute the
		// unicode dash character
		$dash = json_decode('"\u2010"');
		$this->newTitle = preg_replace('@[-]@u', $dash, $this->newTitle);
