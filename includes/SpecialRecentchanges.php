...
function rcNamespaceForm( $namespace, $invert, $reverse, $featured, $nondefaults, $categories_any ) {
	...
	$submitbutton = '<input type="submit" value="' . wfMsgHtml( 'allpagessubmit' ) . "\" class=\"button secondary\" />\n<br/>";
...
<div id='nsselect' class='recentchanges'>
	...
	{$namespaceselect}{$submitbutton}<br />{$invertbox} <label for='nsinvert'>" . wfMsgHtml('invert') . "</label> {$reversebox}<label for='nsreverse'>" . wfMsgHtml('reverseorder') . "</label> {$featuredbox}<label for='nsfeatured'>" . wfMsgHtml('showonlyfeatured') . "</label> {$catbox}\n</div>";
	$out .= '</form></div>';
	return $out;
}
