...
class MessageCache {
	...
	function get( $key, $useDB = true, $forContent = true, $isFullKey = false ) {
		global $wgContLanguageCode, $wgContLang, $wgLang, $wgLanguageCode;
		if( $forContent ) {
			$lang =& $wgContLang;
		} else {
			$lang =& $wgLang;
		...
		if( ($message === false || $message === '-' ) &&
			...
			// Added by Gershon Bialer 07/23/2013 to automatically convert MediaWiki messages stored in db to different Chinese variants
			if(!$forContent &&  $wgLanguageCode == 'zh' && $message !== false) {
