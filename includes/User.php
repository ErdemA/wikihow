...
class User {
	...
	public static function isValidEmailAddr( $addr ) {
		...
		// Code taken from MediaWiki 1.18 Sanitizer::validateEmail function
	...
	function setCookies() {
		...
		$_SESSION['wsToken'] = $this->mToken;
		if ( 1 == $this->getOption( 'rememberpassword' ) || $this->isFacebookUser() || $this->isGPlusUser()) {
...
error_log(__METHOD__ . ": calling Autoblock for user " . $this->mName . " id=" . $this->mId . " ip=" . $currentIP . "\n", 3, '/tmp/block.log');
	...
	function isGPlusUser() {
		if (!$this->mLoadedGPlus) {
			...
			// $dbw = wfGetDB(DB_MASTER);
			// $dbw->selectDB(WH_DATABASE_NAME_SHARED);
			// $count = $dbw->selectField('gplus_connect',
				// array('gplus_user'),
				// array('wh_user' => $this->mId),
				// __METHOD__
			// );
