...
<script type=\"text/javascript\" src=\"{$wgStylePath}/common/upload.js?{$wgStyleVersion}\"></script>
		...
		$wgOut->addWikiMsg( 'uploadtext_new', $this->mDesiredDestName );
		$wgOut->addHTML( "</div>\n" );
		...
		if( $wgAllowCopyUploads && $wgUser->isAllowed( 'upload_by_url' ) ) {
			...
			$filename_form =
				"<input tabindex='1' type='file' class='input_med' name='wpUploadFile' id='wpUploadFile' " .
				($this->mDesiredDestName?"":"onchange='fillDestFilename(\"wpUploadFile\")' ") .
				"size='40' />" .
				"<input type='hidden' name='wpSourceType' value='file' />" ;
		}
		// turn off for now
		if (false &&  $useAjaxDestCheck ) {
	...
	<form id='upload' method='post' enctype='multipart/form-data' action="$action" onsubmit="return checkFFBug();">
		...
		<tr>
	  {$this->uploadFormTextTop}
			<td align='$align1' valign='middle'><label for='wpUploadFile'>{$sourcefilename}:</label></td>
			<td align='$align2'>
				{$filename_form}
			</td>
		</tr>
		<tr>
			...
			<td align='$align2'>
				<input tabindex='2' type='text' name='wpDestFile' id='wpDestFile' class='input_med' 
		...
		<tr>
			...
			<td align='$align2'>
				<textarea tabindex='3' name='wpUploadDescription' id='wpUploadDescription' rows='6' 
					cols='40'{$width} class='input_med'>$encComment</textarea>
	   {$this->uploadFormTextAfterSummary}
			</td>
		</tr>
		<tr>
EOT
		...
		if ( $licenseshtml != '' ) {
			...
			<td align='$align2'>
				<select name='wpLicense' class='input_med' id='wpLicense' tabindex='4'
	...
	<tr>
		<td></td>
		<td align='$align2'><input id='gatWPUpload' tabindex='9' type='submit' class='button primary' name='wpUpload' value=\"{$ulb}\"" . $wgUser->getSkin()->tooltipAndAccesskey( 'upload' ) . " /></td>
	</tr>
	<tr>
		<td></td>
		<td align='$align2'>
