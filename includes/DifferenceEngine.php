...
CONTROL;
		...
		if ($this->mOldRev) {
			...
			'<div class="diff_details">'. $av . 
			'<div id="mw-diff-otitle1a">' . wfMsg('diff_by') . ' ' . $userName . '</div>' .
			'<div id="mw-diff-otitle2">'. $this->mOldDate . "</div>" .
			'<div id="mw-diff-otitle2b">' . $userTools . "</div>";
			if($comment != "")
		...
		$newHeader = '<div id="mw-diff-ntitle1" ' . $th_diff_div . '><h4 ' . $thumbHeader . '>'.$this->mNewtitle . ' ' . $nextlink . '</h4></div>' .
			'<div class="diff_details">' . $av . $thumbsHtml . 
			'<div id="mw-diff-ntitle1a">' . wfMsg('diff_by') . ' ' . $userName . '</div>' .
			'<div id="mw-diff-ntitle2">' . $this->mNewDate . "</div>" .
			'<div id="mw-diff-ntitle2b">' . $userTools . "</div>" .
			'<div id="mw-diff-ntitle4" style="text-align:left">' . $rollback . $nextlink . $patrol . '</div>';
		if($comment != "")
...
//			$wgOut->addWikiTextTidy( $this->mNewtext );
	...
	static function addHeader( $diff, $otitle, $ntitle, $multi = '' ) {
		...
		//next line is to fix the random IE10 but that doesn't display right if there's no diff.
		//for some reason we need one full row with all the <td>'s and no colspans.
		if( $diff == '')
			$header .= "<tr><td></td><td></td><td></td><td></td></tr>";
