...
class OutputPage {
	...
	var $mHTMLtitle, $mRobotpolicy, $mRobotpolicyDebug, $mIsarticle, $mPrintable;
	...
	function addJScode($code) { $this->mJSminCodes[] = $code; }
	function addCSScode($code) { $this->mCSSminCodes[] = $code; }
	...
	public function setRobotpolicy( $str, $dbg = '' ) {
		$this->mRobotpolicy = $str;
		$this->mRobotpolicyDebug = $dbg;
	}
	public function getRobotpolicy() { return $this->mRobotpolicy; }
	...
	public function formatPermissionsErrorMessage( $errors ) {
		...
		if (count( $errors ) > 1) {
			...
			$text .= '<div class="permissions-errors">' . call_user_func_array('wfMsgNoTrans', $err) . '</div>';
		}
	...
	public function addReturnTo( $title ) {
		...
		$this->addHtml( "<p class='loginsuccess'>{$link}</p>\n" );
	}
	...
	public function getHeadLinks() {
		...
		if ($this->mRobotpolicy === '') {
			$policy = 'noindex,nofollow';
			$policyDebug = 'default';
		} else {
			$policy = $this->mRobotpolicy;
			$policyDebug = $this->mRobotpolicyDebug;
		}

		// http://www.robotstxt.org/wc/meta-user.html
		// Only show if it's different from the default robots policy
		$debugText = $policyDebug ? "<!--{$policyDebug}-->" : '';
		$ret .= "<meta name=\"robots\" content=\"$policy\" />$debugText\n";
