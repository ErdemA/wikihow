...
class LanguageConverter {
	...
	*/
	function getPreferredVariant( $fromUser = true ) {
		global $wgUser, $wgRequest, $wgVariantArticlePath, $wgDefaultLanguageVariant, $wgCookiePath, $wgCookieDomain, $wgCookiePrefix;
		...
		// Written by Gershon Bialer 7/2013. 
		// Added cookie support for selecting the language variant variant to allow language variant persistence		
		$cookiename = $wgCookiePrefix.'variant';
		if(isset($_COOKIE[$cookiename]) && in_array($_COOKIE[$cookiename], $this->mVariants )) {
			$this->mPreferredVariant = $_COOKIE[$cookiename];
