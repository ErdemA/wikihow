...
class PopBox {
	...
	function getGuidedEditorButton( ) {
		global $wgUser;
		return "<a class='" . ($wgUser->getID() == 0 ? " disabled" : "") . " id='weave_button' accesskey='" .wfMsg('popbox_accesskey') ."'  onclick='PopItFromGuided(); return false;' href='#' " . ($wgUser->getID() == 0 ? " disabled=\"disabled\" " : "") . ">" . wfMsg('popbox_add_related') . "</a>";
	           
	}
	
	function getToolbarButton( ) {
		 return "<div class='mw-toolbar-editbutton' id='weave_button' accesskey='" .wfMsg('popbox_accesskey') ."' onclick='PopIt(document.editform.wpTextbox1); return false;' title='Weave links'></div>\n"  ;	
	}
	...
	function processResponse() {
		if (requester.status == 0 || requester.status == 200) {
			...
			$('#dialog-box').dialog({
				modal: true,
				width: 400,
				title: '" . wfMsg('popbox_related_articles') . "',
				minHeight: 200,
				closeText: 'Close',
				buttons: [ 
							{ text: \"". wfMsg('popbox_revise') . "\", click: function() { return Revise(); } },
							{ text: \"". wfMsg('popbox_nothanks') . "\", click: function() { $( this ).dialog( \"close\" ); } }
						],
			});
		}
	}
	...
	function Revise() {
		...
		if (document.getElementById('wpTextbox1') && ( (agent.indexOf('firefox') >= 0) || (agent.indexOf('msie 8.0') >= 0) )) {
			...
			$('#dialog-box').html('<p id=\"popbox_inner\"><input id=\"revise_text\" type=\"text\" name=\"revise\" class=\"search_input\" onKeyUp=\"fakeSubmit(event);\" onclick=\"\" value=\"' + searchtext + '\"/><button onclick=\"return searchFormSubmit();\" class=\"search_button\">"  .wfMsg('popbox_search') . "</button></p>');
	...
	function PopIt(obj) {
		...
		if (text == '') {
			html = '<p id=\"popbox_inner\">"  . wfMsg('popbox_no_text_selected') . "</p>';
