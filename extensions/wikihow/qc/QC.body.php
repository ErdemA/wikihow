...
class QCRuleTip extends QCRule {
	...
	function getTipData($tipID) {
		$dbr = wfGetDB(DB_SLAVE);
		$res = $dbr->select('tipsandwarnings_log', array('tw_user','tw_tip','tw_page'), array('tw_id' => $tipID, 'tw_action' => TipsPatrol::TIP_ACTION_QG),__METHOD__,array('LIMIT' => 1));
		foreach ($res as $row) {
			$tip_user = (!empty($row->tw_user)) ? User::newFromID($row->tw_user) : '';
			$the_tip = $row->tw_tip;
			$tip_page = $row->tw_page;
		}
		return array($tip_user, $the_tip, $tip_page);
	}
	...
	function rollbackChange($qcid) {
		...
		TipsPatrol::deleteTip($tipid, $tip_page, $the_tip, false, true);
		return true;
	}
