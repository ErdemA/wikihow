...
class Quizzes extends UnlistedSpecialPage {
	...
	function __construct() {
		UnlistedSpecialPage::UnlistedSpecialPage( 'Quizzes' );
	}
	...
	function GrabQuizCTA($quiz_name, $t) {
		...
		// global $wgUser;
		// $sk = $wgUser->getSkin();
		// $img = $sk->getGalleryImage($t, 250, -1, true); //skip the parser
		...
		$html = '<div class="quiz_cta_2">'.
				'<a href="/Quiz/'.$quiz_name.'"><img src="'.wfGetPad('/images/thumb/9/95/Make-Your-Butt-Look-Bigger-Quiz.png/170px-Make-Your-Butt-Look-Bigger-Quiz.png').'" /></a><br />'.
				'<a href="/Quiz/'.$quiz_name.'">'.$quiz_display_name.' '.wfMsg('quiz-suffix').'</a>'.
				'</div>'.
				'<br class="clearall" />';
	...
	private function getBoxBgImg($db, $quiz_name, $width, $height) {
		...
		if ($t) {
			$sk = $wgUser->getSkin();
			$img = $sk->getGalleryImage($t, $width, $height);
			$img = wfGetPad($img);
		}
