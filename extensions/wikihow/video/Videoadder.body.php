...
class Videoadder extends SpecialPage {
    ...
    function execute ($par) {
		...
		if ($target == 'getnext') {
			...
			$html = WikihowArticleHTML::processArticleHTML($parserOutput, array('no-ads', 'ns' => NS_MAIN));
...
<object width='480' height='385'><param name='movie' value='http://www.youtube.com/v/{$id}&amp;hl=en_US&amp;fs=1'></param><param name='allowFullScreen' value='true'></param><param name='allowscriptaccess' value='always'></param><embed src='http://www.youtube.com/v/{$id}&amp;hl=en_US&amp;fs=1' type='application/x-shockwave-flash' allowscriptaccess='always' allowfullscreen='true' width='480' height='385'></embed></object>
		...
		$this->setHeaders();
		$this->setSideWidgets();
		$wgOut->addScript('<script type="text/javascript" src="/extensions/min/f/extensions/wikihow/video/cookie.js,/extensions/wikihow/video/videoadder.js?rev='.WH_SITEREV.'"></script>');
		$wgOut->addHTML('<style type="text/css" media="all">/*<![CDATA[*/ @import "/extensions/min/f/extensions/wikihow/video/videoadder.css"; /*]]>*/</style>');
		$wgOut->addHTML("<div class='tool_header tool'><h1>" . wfMsg('va_question') . "<span id='va_instructions'>" . wfMsgExt('va_instructions', 'parseinline') . "</span></h1>");
