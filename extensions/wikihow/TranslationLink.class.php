...
class TranslationLink {
	...
	*/
	static function getLinksTo($fromLang, $fromPageId, $getTitles = true) { 
		$dbr = wfGetDB(DB_SLAVE);
		$sql = "select tl_from_lang,tl_from_aid,tl_to_lang,tl_to_aid from wikidb_112.translation_link where (tl_from_lang=" . $dbr->addQuotes($fromLang) . " AND tl_from_aid=" . $dbr->addQuotes($fromPageId) . ") OR (tl_to_lang=" . $dbr->addQuotes($fromLang) . " AND tl_to_aid=" . $dbr->addQuotes($fromPageId) . ")";
		$res = $dbr->query($sql, __METHOD__);
		$tls = array();
