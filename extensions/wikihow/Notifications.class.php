...
class Notifications {
		...
		if (!is_array($box)) {
			...
			if ($wgUser->getNewtalk()) {
				...
				$msg = '<div class="note_row"><div class="note_icon_talk"></div>' .
						'<a href="/'.$wgUser->getTalkPage().'#post">'.wfMsg($m,$talk_count).'</a></div>';
				$notes[] = $msg;
				$bNewTalk = true;
			}
			else {
			...
			if ($wgUser->getNewkudos() && !$wgUser->getOption('ignorefanmail')) {
				...
				$msg = '<div class="note_row"><div class="note_icon_kudo"></div>' .
						'<a href="/'.$wgUser->getKudosPage().'#post">'.wfMsg($m,$kudos_count).'</a></div>';
				$notes[] = $msg;
			}
	...
	private function formatNotifications($notes,$bNewTalk) {
		...
		if ($html) {
			...
			if (!$bNewTalk) {
				$html .= '<div class="note_row note_empty">'.wfMsg('notifications_notalk', $talkPageUrl).'</div>';
			}
		}
		else {
			$html = '<div class="note_row note_empty">'.wfMsg('notifications_none', $talkPageUrl).'</div>';
		}
