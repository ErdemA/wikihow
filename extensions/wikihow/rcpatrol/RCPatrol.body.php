...
class RCPatrol extends SpecialPage {
	...
	private static function generateRollback($rev, $oldid = 0) {
	...
	static function getButtons($result, $rev, $rcTest = null) {
		...
		$s .= self::generateRollback($rev, $result['old']);
		$s .= "</td></tr></table>";
		$s .= "<div id='rc_subtabs'>
...
class RCPatrolData {
	...
	static function getNextArticleToPatrolInner($rcid = null) {
		...
		$rollbackCommentPrefix = wfMsgForContentNoTrans('rollback_comment_prefix');
		...
		$cat = "";
		$userContentFilter = $wgUser->getOption('contentfilter');
		if ($userContentFilter > 0) {
			if ($userContentFilter == 1) $cat = " AND page_catinfo & " . CAT_TEEN . " = " . CAT_TEEN;
			if ($userContentFilter == 2) $cat = " AND page_catinfo & " . CAT_TEEN . " = 0 ";
		}
...
exit;
		...
		if ($row) {
			...
			// if the article is new, set 'old' to -1 so DifferenceEngine 
			// doesn't just show last edit
	...
	/*static function getGlobalPatrolRankings() {
		global $wgMemc;
	...
	/*static function getQuickEditsToday() {
		global $wgUser;
	...
	/*static function getUnPatrolledEdits() {
		$dbr = wfGetDB(DB_SLAVE);
		$count = $dbr->selectField('recentchanges',
			array('count(*)'),
	...
	/*static function getEditsPatrolled($alltime) {
		global $wgUser;
		$dbr = wfGetDB(DB_SLAVE);
...
class RCTestStub {
	...
	static function getDifferenceEngine($result, &$rcTest) {
		if (class_exists('RCTest') && RCTest::isEnabled()) {
