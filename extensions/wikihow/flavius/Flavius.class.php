...
class Flavius {
	...
	private $db;
	// Database host for the Flavius database
	const FLAVIUS_DB_HOST = "127.0.0.1";
	...
	public function __construct() {
		$this->db = new Database(self::FLAVIUS_DB_HOST, WH_DATABASE_MAINTENANCE_USER, WH_DATABASE_MAINTENANCE_PASSWORD, self::DATABASE_NAME);
	 ...
	 */
	public function calcIntervalStats(&$ids, $stats, $startDate, $endDate) {
	...
	public function shiftTotals($endDate) {
		...
		$sql = 'insert into flavius_total(ft_user, ft_field, ft_value, ft_end_date, ft_date_calculated) select ft_user, ft_field, ft_value+if(total is NULL,0,total) as ft_value,' . $this->db->addQuotes($endDate) . " as ft_end_date,date_format(now(),'%Y%m%d%h%i%s') as ft_date_calculated from flavius_total left join (select fi_user, fi_field, sum(fi_value) as total from flavius_interval WHERE fi_day >" . $this->db->addQuotes($lastTotalDate) . ' AND fi_day <=' . $this->db->addQuotes($endDate) . ' group by fi_user, fi_field) fis on ft_user=fi_user AND ft_field=fi_field where ft_end_date=' . $this->db->addQuotes($lastTotalDate) . " GROUP BY ft_user, ft_field, ft_end_date" ;
		$this->db->query($sql, __METHOD__);
...
class FContributionEditCount extends FSInterval {
	public function batchCalcInterval(&$dbr, &$users, $startDate, $endDate) {
		...
		$userTotal = array();
		if($totalDate) {
			$sql = "select ft_user, ft_end_date, ft_value FROM flavius.flavius_total WHERE ft_end_date = " . $dbr->addQuotes($totalDate) . " AND ft_field='contribution_edit_count'";
			$res = $dbr->query($sql, __METHOD__);
			foreach($res as $row) {
				$userTotal[$row->ft_user] = $row->ft_value;	
			}
			$sql = "select fi_user, fi_day, sum(fi_value) as s from flavius.flavius_interval where fi_day >= " . $dbr->addQuotes($totalDate) .  " and fi_day < " . $dbr->addQuotes($endDate) . " and fi_field='contribution_edit_count' GROUP BY fi_user";
		}
		else {
			$sql = 	"select fi_user, fi_day, sum(fi_value) as s from flavius.flavius_interval where fi_day < " . $dbr->addQuotes($endDate) . " and fi_field='contribution_edit_count' GROUP BY fi_user";
...
class FTalkPagesReceived extends FSInterval {
	...
	function batchCalcInterval(&$dbr, &$users, $startDate, $endDate) {
		$ids = $this->getIds($users);
		$sql = "select user_id," . $this->getDayQuery("rev_timestamp") . ",count(*) as ct from revision join page on rev_page=page_id join wiki_shared.user on replace(user_name,' ','-') = page_title where page_namespace=" . NS_USER_TALK . " AND rev_timestamp > " . $dbr->addQuotes($startDate) . " AND rev_timestamp<= " . $dbr->addQuotes($endDate) . " AND rev_user in (" . implode(",",$ids) . ") GROUP BY day, user_id";
		$res = $dbr->query($sql, __METHOD__);
		$ret = array();
		foreach($res as $row) {
	...
	function batchCalcTotals(&$dbr, &$users, $endDate) {
		$ids = $this->getIds($users);
		$sql = "select user_id,count(*) as ct from revision join page on rev_page=page_id join wiki_shared.user on replace(user_name,' ','-') = page_title where page_namespace=" . NS_USER_TALK . " AND rev_timestamp <= " . $dbr->addQuotes($endDate) . " AND user_id in (" . implode(',',$ids) .  ") GROUP BY user_id";
		$res = $dbr->query($sql, __METHOD__);
		$ret = array();
		foreach($res as $row) {
