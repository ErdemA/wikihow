...
class DocViewer extends UnlistedSpecialPage {
	 ...
	 */
	public static function getPageTitle($doc_name='') {
	...
	public static function getDisplayName($sample) {
		global $wgMemc;
		$memkey = wfMemcKey('sample_display_name_'.$sample);
		...
		if (!$name) {
			...
			if (!$name) {
				//default
				$name = wfMsg('dv-sample-prefix').' '.str_replace('-',' ',$sample);			
			}
	...
	public static function displayContainer($doc_name='',$is_mobile) {
		...
		if ($doc_name) {		
			...
			$tmpl->set_vars(array(
				...
				'header_get' => wfMsg('header_get'),				
				'header_found' => wfMsg('header_found'),
				'header_related' => wfMsg('header_related'),
				'show_staff_stats' => self::showStaffStats(),
				'dv_download' => wfMsg('dv-download'),
				...
				'doc_name' => $doc_name,
				'dv_open_in' => wfMsg('dv-open-in'), 
				'dv_dl_text_pdf' => wfMsg('dv-dl-text-pdf'), 
				'dv_dl_file_pdf' => $doc_array['pdf'], 
				'dv_display_pdf' => 'http://www.wikihow.com/Sample/'.str_replace($doc_array['pdf'],'sampledocs','samplepdfs'), 
				'dv_dl_text_doc' => wfMsg('dv-dl-text-doc'), 
				'dv_dl_file_doc' => $doc_array['doc'], 
				'dv_dl_text_xls' => wfMsg('dv-dl-text-xls'), 
				'dv_dl_file_xls' => $doc_array['xls'], 
				'dv_dl_text_txt' => wfMsg('dv-dl-text-txt'), 
				'dv_dl_file_txt' => $doc_array['txt'], 
				'dv_dl_text_gdoc' => wfMsg('dv-dl-text-gdoc'), 
		...
		foreach ($res as $row) {
			...
			if($t && $t->exists()) {
                ...
                if($is_mobile)
    				...
    				$html .=  $sk->getArticleThumb($t, 127, 140);

				//save for meta description
				if (!self::$firstRelated) {
					self::$firstRelated = 	' '.wfMsg('dv-meta-article-prefix').
	...
	function GrabOneDoc($doc_name) {
		...
		if ($file && isset($file)) {
			...
			$html = '<div class="sd_thumb">'.
					'<a href="/Sample/'.$doc_name_hyphenized.'"><img src="'.wfGetPad($thumb->url).'" /></a>'.
	...
	function execute($par = '') {
		...
		//the guts
		$wgOut->addCSScode('dvc'); // css path defined in extensions/min/groupConfig.php
