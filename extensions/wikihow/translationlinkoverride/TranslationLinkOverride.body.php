...
class TranslationLinkOverride extends UnlistedSpecialPage {
	...
	function __construct() {
		UnlistedSpecialPage::UnlistedSpecialPage('TranslationLinkOverride');
	}
	 ...
	 */
	function parseURLlist($pageList) {
		$pageList = preg_split("@[\r\n]+@", $pageList);
		$urls = array();
		foreach ($pageList as $line) {
			$both = preg_split("@\t@", $line);
			if(count($both) ==2) {
				$urls[] = array('url1' => urldecode($both[0]),'url1_raw' => $both[0], 'url2' => urldecode($both[1]), 'url2_raw' => $both[1]);
			}
		}
		return($urls);
	}
	 ...
	 */
	function doDelete($fromLang, $fromId, $toLang, $toId) {
		global $wgOut;
		TranslationLink::writeLog(TranslationLink::ACTION_DELETE, $fromLang, NULL, $fromId, NULL, $toLang, "unknown", $toId, "TranslationLinkOverride");
		...
		$wgOut->setArticleBodyOnly(true);
		if($link->delete()) {
			$wgOut->addHTML(json_encode(array('success'=>true)));
		}
		else {
			$wgOut->addHTML(json_encode(array('success'=>false)));
		}
	}
	/**
	 * EXECUTE
