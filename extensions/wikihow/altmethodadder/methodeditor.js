...
$('#method_header').one('keyup', '#method_method', function(e) {
    $('#method_read').attr('checked', true);
});
$('#method_header').one('keyup', '#method_steps', function(e) {
    $('#method_read').attr('checked', true);
});
...
$( document ).on( "click", "#method_keep", function(e) {
	...
	if (!jQuery(this).hasClass('clickfail')) {
        if (!$('#method_read').is(':checked')) {
            alert("Did you mean to publish this alternate method without editing it? If you did please check the box above the 'Skip' button.");
            return false;
        }
		clearTool();
		$.post(toolURL, {
...
function loadResult(result) {
	$("#method_waiting").hide();
    $('#method_read').attr('checked', false);
	if (result['error']) {
		$("#method").hide();
		$("#method_error").show();
		$("#method_count").hide();
