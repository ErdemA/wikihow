...
class NFDProcessor {
	...
	function getNextToPatrolHTML() {
		...
		if (!$t) {
			NFDProcessor::markPreviousAsInactive($this->mResult->nfd_page);
		...
		$tmpl->set_vars(array(
			...
			'articleHtml' => WikihowArticleHTML::processArticleHTML($wgOut->parse($r->getText(), $t, $popts), array('no-ads'=>1, 'ns' => $t->getNamespace())),
	...
	function setFirstEdit(){
		...
		$this->mFirstEdit = $dbr->selectField('firstedit', array('fe_timestamp'), array('fe_page'=> $this->mArticle->getID()), __METHOD__);
	}
	...
	function getVotes($nfdid, &$dbr){
		...
		$votes['keepUsers'] = "";
		$votes['deleteUsers'] = "";
		$res = $dbr->select('nfd_vote', '*', array('nfdv_nfdid' => $nfdid), __METHOD__);
...
class NFDGuardian extends SpecialPage {
	...
	function getNfdReasonsDropdown($defaultValue='all'){
		$html = "<div id='nfd_reasons' class='tool_options'><span>" . wfMsg('nfd_dropdown_text') . "<select>";
