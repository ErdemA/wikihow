...
class WelcomeWagon extends UnlistedSpecialPage {
	...
	function addSummary($targetUser) {
		...
		$pager = new WelcomeWagonContribsPager($target);
		if ( !$pager->getNumRows() ) {
	...
	function skipUser($userId) {
		...
		$key = intval($userId);
		if ($skips && $skips[$key] != null) {
			$skips[$key] = $skips[$key] + 1;
			$wgMemc->set($this->userSkipsKey, $skips, 60*60*24);

			if ($skips[$key] > $this->maxSkipPerUser) {
				$userIds = $wgMemc->get($this->userIdsKey);
				if ($userIds[$key] == 1) {
					$userIds[$key] = 0;
					$wgMemc->set($this->userIdsKey, $userIds, 60*60*24);
				}
			}
		}
	}
	...
	function messagedUser($toId) {
		...
		if ($sent && $sent[$key] != null) {
			$sent[$key] = $sent[$key] + 1;
			$wgMemc->set($this->userMessagesKey, $sent, 60*60*24);
	...
	function resetCache() {
		...
		$wgMemc->set($this->userMessagesKey, $messagesSent, 60*60*24);
		$wgMemc->set($this->userSkipsKey, $skips, 60*60*24);
		$wgMemc->set($this->userIdsKey, $userIds, 60*60*24);
