...
class ProposedWhitelist extends UnlistedSpecialPage {
	...
	function __construct() {
		UnlistedSpecialPage::UnlistedSpecialPage('ProposedWhitelist');
	}
	
	function execute($par) {
		global $wgOut, $wgUser, $wgMemc, $wgRequest;
		
		if($wgUser->getID() == 0 || !($wgUser->isSysop() || in_array( 'newarticlepatrol', $wgUser->getRights() ) || $wgUser->getName() == "Gloster flyer" )) {
			$wgOut->setRobotpolicy( 'noindex,nofollow' );
			$wgOut->errorpage( 'nosuchspecialpage', 'nospecialpagetext' );
			return;
		}
