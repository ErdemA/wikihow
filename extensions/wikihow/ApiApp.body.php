...
class AppDataFormatter {
	...
	public static function getThumbnailDimensions($thumb) {
		...
		if (!$thumb || !$thumb->file) {
			$result['width'] = 0;
			$result['height'] = 0;
		} else if($thumb->file->path == $thumb->path) {
			$result['width'] = intval($thumb->file->width);
			$result['height'] = intval($thumb->file->height);
		} else {
			$result['width'] = intval($thumb->width);
			$result['height'] = intval($thumb->height);
		}
		return $result;
	}
	...
	public static function getImageDetails($image) {
		...
		if ($image) {
			...
			if ($thumb && !($thumb instanceof MediaTransformError)) {
				$result['url'] = self::uriencode(wfGetPad($thumb->url));
			...
			if ($thumb && !($thumb instanceof MediaTransformError)) {
				$largeUrl = self::uriencode(wfGetPad($thumb->url));
				if ($largeUrl != $result['url']) {
					$result['large'] = self::uriencode(wfGetPad($thumb->url));
					$dim = self::getThumbnailDimensions($thumb);
			...
			if ($original != $largeUrl) {
				$result['original'] = self::uriencode(wfGetPad($image->url));
				$result['original_width'] = intval($image->width);
				$result['original_height'] = intval($image->height);
			}
		}
		return $result;
	}
	...
	static function addImageToIntro(&$parsed, $title) {
		...
		if ($image) {
			$image = AppDataFormatter::getImageDetails($image);
		}
...
class ApiSectionParser {
	...
	private function pullOutImage(&$imgNode, $remove = true) {
		$imgUrl = $imgNode->attr('href');
		$imgName = preg_replace('@^/(Image|' . $this->imageNsText . '):@', '', $imgUrl);
		$image = self::getImageObj($imgName);
		$ret = $image ? AppDataFormatter::getImageDetails($image) : null;
	...
	private function pullOutWikiVideo(&$whvid) {
		$ret = null;
		if ($whvid->length) {
			$lrgimg = AppDataFormatter::uriencode(trim( pq('div[id^=lrgimgurl]', $whvid)->text() ));
			$smlimg = AppDataFormatter::uriencode(trim( pq('div[id^=smlimgurl]', $whvid)->text() ));
			$vid = AppDataFormatter::uriencode(trim( pq('div[id^=vidurl-whvid]', $whvid)->text() ));
	...
	private function removeEmptyNodes($stepNode) {
	...
	private function processStepContent($node) {
		...
		foreach (pq('div.step_content', $node) as $stepNode) {
			...
			if ($imgNode->length) {
				$image = $this->pullOutImage($imgNode);
				if ($image) {
					$step['image'] = $image;
				}
			}
			...
			$this->removeEmptyNodes(&$stepNode);
	...
	private function randomDivId($depth=0) {
		if ($depth > 10) {
		...
		if ($node->length() > 0 || empty($id)) {
			return $this->randomDivId($depth+1);
		}
		return $id;
	}

	// processes a section type before a specified next element
	// used for processing the intro but not sure if it will be reused yet
	private function processIntro($before) {
		$result = array('html'=>'');
		$id = $this->randomDivId();
...
DONE;
	...
	private static function getImageObj($imgName) {
		...
		$tmpTitle = Title::newFromURL($imgName);
		if (!$tmpTitle) {
