<? if (isset($scriptsCombine1) && is_array($scriptsCombine1)): ?>
	<? if ($deviceOpts['name'] != 'iphoneapp'): ?>
		<script type="text/javascript" src="/extensions/min/g/<?= join(',', $scriptsCombine1) ?><?= isset($GLOBALS['wgRequest']) && $GLOBALS['wgRequest']->getVal('c') == 't' ? '&c=t' : '' ?>&r=<?= WH_SITEREV ?>&e=.js"></script>
	<? else: ?>
		<? // HACK FOR OLD iPhone APP ?>
		<script type="text/javascript" src="<?= wfGetPad() ?>/extensions/min/?g=<?= join(',', $scriptsCombine1) ?>&rev=<?= WH_SITEREV ?>"></script>
	<? endif; ?>
<? endif; ?>
