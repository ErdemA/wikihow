...
class WikihowArticleStream {
	...
	function __construct($articleStream, $startArticle = 0) {
			 ...
			 $i++)
		{
			...
			if (preg_match('@title="([^"]+)"@', $link, $matches)) {
				...
				if ($title) {
					...
					if (!($image && $image->getPath() && file_exists($image->getPath()) && strpos($image->getPath(), "?") === false) 
			...
			if ($articles) {
				...
				foreach ($layout as $item) {
					if (!$item['title']) {
						...
						$html .= $skin->getArticleThumbWithPath($item['title'], $xUnits*$singleWidth + $singleSpacing*($xUnits-1), $yUnits*$singleHeight + $singleSpacing*($yUnits-1), $item['image']);
						$html .= "</td>";
