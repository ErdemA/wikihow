...
class HillaryRest extends UnlistedSpecialPage {
	...
	public function execute($par) {
		...
		if ($action) {
			...
			$toolURL = 'http://' . MobileWikihow::getMobileSite() . '/' . $next_url . $utmParamsStr . '#review';
...
class Hillary {
	...
	public static function onDelete($article) {
		if ($article) {
...
EOHTML;
	...
	public function getActionArticles() {
		$db = wfGetDB(DB_SLAVE);
		$res = $db->select('hillary_pages',
			array('*'),
			array("hp_action <> ''"),
	...
	public function extractArticles(&$res) {
		$articles = array();
		foreach ($res as $row) {
	...
	public function getProgressArticles() {
		$db = wfGetDB(DB_SLAVE);
		$res = $db->select('hillary_pages',
			array('*'),
			array("hp_action = ''"),
