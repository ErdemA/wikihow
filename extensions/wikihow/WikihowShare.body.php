...
class WikihowShare {
	...
	public static function getMainPageShareButtons(){
		...
		$action = self::getAction();
		if(!$wgTitle->exists() || $wgTitle->getNamespace() != NS_MAIN || $action != "view" || $wgTitle->getText() != wfMsg('Mainpage'))
			return "";
		
		return wfMsg('mainpage_share_buttons');
	...
	public static function getBottomShareButtons($isIndexed = true) {
		...
		// German includes "how to " in the title text
		$howto = $wgLanguageCode != 'de' ? wfMsg('Twitter_text', htmlspecialchars($wgTitle->getText())) : htmlspecialchars($wgTitle->getText());
...
class WikihowShareRest extends UnlistedSpecialPage {
    public function __construct() {
		UnlistedSpecialPage::UnlistedSpecialPage( 'WikihowShare' );
	}
