...
class RobotPolicy {
	...
	public static function setRobotPolicy() {
		...
		if ($wgOut && $wgTitle) {
			...
			case self::POLICY_NOINDEX_FOLLOW:
				$wgOut->setRobotPolicy(self::POLICY_NOINDEX_FOLLOW_STR, $policyText);
				break;
			case self::POLICY_NOINDEX_NOFOLLOW:
				$wgOut->setRobotPolicy(self::POLICY_NOINDEX_NOFOLLOW_STR, $policyText);
				break;
			case self::POLICY_INDEX_FOLLOW:
				$wgOut->setRobotPolicy(self::POLICY_INDEX_FOLLOW_STR, $policyText);
				break;
			case self::POLICY_DONT_CHANGE:
				$oldPolicy = $wgOut->getRobotPolicy();
				$wgOut->setRobotPolicy($oldPolicy, $policyText);
				break;
			}
		}
		return true;
		...
		if ($articleID) {
			...
			if ($result === null) {
				...
				$sql = "SELECT COUNT(*) AS count FROM templatelinks 
						...
						tl_title IN ('Speedy', 'Stub', 'Copyvio','Copyviobot','Copyedit','Cleanup','Notifiedcopyviobot')";
	...
	*/
	...
	private function inWhitelist() {
		...
		if (!$whitelist) $whitelist = wfMsg('index-whitelist');
		$urls = split("\n", $whitelist);
		foreach ($urls as $url) {
			$url = trim($url);
			if ($url) {
