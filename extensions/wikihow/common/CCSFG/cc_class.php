<?php
    ...
    */
    ...
    class CC_Utility {
        ...
        var $login = WH_CONSTANTCONTACT_USERNAME; //Username for your account
        var $password = WH_CONSTANTCONTACT_PASSWORD; //Password for your account
        var $apikey = WH_CONSTANTCONTACT_KEY; // API Key for your account.
    ...
    */
     ...
     */ 
	 ...
	 public function subscriberCanSubscribe($email = '') {
		 $call = $this->apiPath.'/contacts?email='.$email;
		 $return = $this->doServerCall($call);
		 $xml = simplexml_load_string($return);
		 $id = $xml->entry->id;
		 if($id && $xml->entry->content->Contact->status == "Removed"){ return $id; }
		 else { return false; }
	 }
