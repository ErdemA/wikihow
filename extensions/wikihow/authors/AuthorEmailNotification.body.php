...
class AuthorEmailNotification extends SpecialPage {
	...
	function reassignArticleAnon($articleid) {
		...
		$ret = $dbw->update('firstedit',
			array('fe_user_text' => $wgUser->getName(),
	...
	function getEditUserHtml(&$user) {
		...
		if ($user->getId() != 0) {
			...
			$talkPageUrl = 'http://www.wikihow.com' . $user->getTalkPage()->getLocalURL() . $track_talk;
...
*/
	...
	function addUserWatch($target, $watch) {
		...
		$sql = 'INSERT INTO email_notifications (en_user, en_page, en_watch) VALUES (' .
			$dbw->makeList( array($wgUser->getID(), $target, $watch) ) .
	...
	function addUserWatchBulk($articles) {
		...
		foreach ($articles as $article) {
			$sql = 'INSERT INTO email_notifications (en_user, en_page, en_watch) VALUES (' .
				$dbw->makeList( array($wgUser->getID(), $article, 1) ) .
	...
	function getCTALink($campaign, $medium) {
		...
		$urlParam = "autoredirect={$title}&utm_source={$campaign}&utm_medium={$medium}&utm_campaign={$campaign}&utm_term={$term}";
