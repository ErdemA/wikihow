...
class ArticleAuthors {
	...
	static function getAuthorHeader() {
		...
		if ($wgUser->getID() > 0) {
			...
			if (!empty($users)) {
				$html = wfMsg('originated_by', "<span id='loggedin'>" .  self::formatAuthorList($users, true, true, 4) . "</span>");
			}
		} else {
			//$users = array_slice(self::$authorsCache, 0, min(sizeof(self::$authorsCache), 4));
			$users =  self::$authorsCache;
			if (!empty($users)) {
				...
				if ($otherUserCount > 1) {
					$html = wfMsg('originated_by_and_others_anon', $authorSpan, $otherUserCount);
				} elseif ($otherUserCount == 1) {
					$html = wfMsg('originated_by_and_1_other_anon', $authorSpan);
				} else {
					$html = wfMsg('originated_by_anon', $authorSpan);
				}
			}
		}
	...
	static function formatAuthorList($authors, $showAllLink = true, $link = true, $max = null) {
		...
		foreach ($authors as $u => $p) {
			if ($u == 'anonymous') {
				$links[] = $link ? "<a href='/wikiHow:Anonymous'>" . wfMsg('anonymous') . "</a>" : wfMsg('anonymous');
			} else {
		...
		if ($showAllLink) {
			$sk = $wgUser->getSkin();
			$html .=  " (" . $sk->makeLinkObj($wgTitle, wfMsg('see_all'), "action=credits")  . ")";
		}
