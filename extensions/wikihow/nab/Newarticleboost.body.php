...
class Newarticleboost extends SpecialPage {
	...
	private function displayNABList(&$dbw, &$dbr) {
		...
		while ($row = $dbr->fetchObject($res)) {
			...
			if ($row->st_title != null) {
				$wgOut->addHTML("<img src='" . wfGetPad('/skins/WikiHow/images/checkmark.png') . "' height='16' width='16' alt='suggestion'/>");
			}
			$wgOut->addHTML("</td><td>");
	...
	private function displayNABConsole(&$dbw, &$dbr, $target) {
		...
		$html = WikihowArticleHTML::processArticleHTML($parserOutput, array('no-ads', 'ns' => $title->getNamespace()));
		$wgOut->addHTML($html);
		$wgOut->addHTML("</div>");
