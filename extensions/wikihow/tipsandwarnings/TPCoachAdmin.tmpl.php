	...
	<table class="tpc_scores">
		...
		?>
			<tr class="<?=$class?>" id='<?=$result['user_id']?>'>
				...
				<td><a href='#' class='reset_test' userId=<?=$result['user_id']?>>Reset</td>
				<td><a href='/Special:UnpatrolTips?target=<?=$result['user_id']?>' target="_blank">Special:UnpatrolTips</td>
				<td><a href='#' class='blockuser' userId=<?=$result['user_id']?>><?=$result['block']?></td>
			</tr>
		<? endforeach; ?>
	</table>
</div>
