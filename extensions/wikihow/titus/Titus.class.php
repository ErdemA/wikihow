...
//Make sure we don't impact production memcached servers for Titus run
if (@$_SERVER['HOSTNAME'] == "spare1.wikihow.com") {
	global $wgMemc;
	define(TITUS_MEMCACHED_SERVER, "127.0.0.1:11211");
	$wgMemc->set_servers(array(TITUS_MEMCACHED_SERVER));
}
	...
	*/
	private function getTitusDB() {
		if (is_null($this->titusDB) || !$this->titusDB->ping()) {
			$this->titusDB = new Database(TITUS_DB_HOST, WH_DATABASE_MAINTENANCE_USER, WH_DATABASE_MAINTENANCE_PASSWORD, self::getDBName());
		}
		return $this->titusDB;
	}
	...
	*/
	private function getWikiDB() {
		...
		if (is_null($this->wikiDB) || !$this->wikiDB->ping()) {
			$this->wikiDB = new Database(TITUS_DB_HOST, WH_DATABASE_MAINTENANCE_USER, WH_DATABASE_MAINTENANCE_PASSWORD, $wgDBname);
		}
		return $this->wikiDB;
	}
...
abstract class TitusStat {
	...
	function checkForRedirects(&$dbr, &$ids) {
		$query = "select page_id,page_title  from wikidb_112.page where page_is_redirect=1 AND page_id in (" . implode($ids,",") . ")";
	...
	function checkForMissing(&$dbr, &$ids) {
		$query = "select page_id  from wikidb_112.page where page_id in (" . implode($ids,",") . ")";
		$res = $dbr->query($query,__METHOD__);
		$foundIds = array();
		foreach($res as $row) {
...
class TSLangLinks extends TitusStat {
  ...
  public function calc(&$dbr, &$r, &$t, &$pageRow) {
		global $wgLanguageCode;
    $sql = "select p.page_title FROM wikidb_112.langlinks JOIN wikidb_112.page p on p.page_id=ll_from WHERE ll_lang='" . $wgLanguageCode . "' AND ll_title='" . $dbr->strencode($t->getText()) . "'";
    $res = $dbr->query($sql);
    $urls = array();
...
class TSTranslations extends TitusStat {
	...
	public function getPageIdsToCalc(&$dbr, $date) {
		...
		$sql = "select distinct tl_from_aid from wikidb_112.translation_link where tl_from_lang=" . $dbr->addQuotes($wgLanguageCode) . " AND tl_timestamp > " . $dbr->addQuotes($start);
		$res = $dbr->query($sql, __METHOD__);
		$ids = array();
		foreach($res as $row) {
			$ids[] = $row->tl_from_aid;	
		}
		$sql = "select distinct tl_to_aid from wikidb_112.translation_link where tl_to_lang=" . $dbr->addQuotes($wgLanguageCode) . " AND tl_timestamp > " . $dbr->addQuotes($start);
		$res = $dbr->query($sql, __METHOD__);
		foreach($res as $row) {
			$ids[] = $row->tl_to_aid;	
		}
		$sql = "select distinct tll_from_aid from wikidb_112.translation_link_log where tll_from_lang=" . $dbr->addQuotes($wgLanguageCode) . " AND tll_timestamp>" . $dbr->addQuotes($start) . " AND NOT (tll_from_aid is NULL)";
		$res = $dbr->query($sql, __METHOD__);
		foreach($res as $row) {
			$ids[] = $row->tll_from_aid;	
		}

		$sql = "select distinct tll_to_aid from wikidb_112.translation_link_log where tll_to_lang=" . $dbr->addQuotes($wgLanguageCode) . " AND tll_timestamp>" . $dbr->addQuotes($start) . " AND NOT (tll_to_aid is NULL)";
		$res = $dbr->query($sql, __METHOD__);
		foreach($res as $row) {
			$ids[] = $row->tll_to_aid;	
		}
...
class TSTop10k extends TitusStat {
	...
	private function getSpreadsheet(&$dbr) {
		...
		try {
			...
			$cols = $gs->getCols(WH_TITUS_TOP10K_GOOGLE_DOC,1,2,2);
			$urlList = array();	
			$pageIds = array();
			$dups = "";
			foreach($cols as $col) {
	...
	public function calc(&$dbr, &$r, &$t, &$pageRow) {
		...
		$ret =array('ti_top10k'=>'', 'ti_is_top10k'=>0);
...
class TSBabelfishData extends TitusStat {
	...
	public function calc(&$dbr, &$r, &$t, &$pageRow) {
		...
		if($wgLanguageCode != 'en') {
			$sql = "select ct_rank, ct_score FROM wikidb_112.babelfish_articles " 
		. " JOIN wikidb_112.translation_link on tl_from_lang='en' AND tl_from_aid = ct_page_id AND tl_to_lang=ct_lang_code"
		. " WHERE tl_to_aid=" . $dbr->addQuotes($pageRow->page_id) . " AND ct_lang_code=" . $dbr->addQuotes($wgLanguageCode);
			...
			$sql = "select ct_rank, ct_score FROM wikidb_112.babelfish_articles WHERE ct_page_id=" . $dbr->addQuotes($pageRow->page_id) . " AND ct_lang_code='es'";	
			$res = $dbr->query($sql, __METHOD__);
			if($row = $dbr->fetchObject($res)) {
				$ret['ti_babelfish_rank'] = $row->ct_rank;
				$ret['ti_babelfish_score'] = $row->ct_score;
