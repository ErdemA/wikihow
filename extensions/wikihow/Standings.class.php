...
class TipsPatrolStandingsIndividual extends StandingsIndividual {
	...
	function getTitle() {
		return wfMsg('tipspatrol_stats_title');
	}
...
class WelcomeWagonStandingsIndividual extends StandingsIndividual {
	...
	function getTitle() {
		...
		return wfMsg('welcomewag_stats_title');
	}
...
class MethodGuardianStandingsIndividual extends StandingsIndividual {
	...
	function getTitle() {
		return wfMsg('methodguardian_stats_title');
	}
...
class MethodEditorStandingsIndividual extends StandingsIndividual {
    ...
    function __construct() {
        $this->mLeaderboardKey = "methodeditortool_indiv1";
    }
    ...
    function getTitle() {
        return wfMsg('methodeditor_stats_title');
    }
...
class TipsPatrolStandingsGroup extends StandingsGroup  {
	function __construct() {
		$this->mCacheKey = "tiptool_standings2";
	}
	...
	function getTitle() {
		return wfMsg('tipspatrol_leaderboard_title');
	}
}

class WelcomeWagonStandingsGroup extends StandingsGroup  {
	function __construct() {
		$this->mCacheKey = "welcomewagon_standings2";
	}

	function getSQL($ts) {
		$sql = "SELECT user_name, count(*) as C ".
			"FROM welcome_wagon_messages left join wiki_shared.user on ww_from_user_id = user_id ".
	...
	function getTitle() {
		return wfMsg('welcomewag_leaderboard_title');
	}
}

class MethodGuardianStandingsGroup extends StandingsGroup  {
	function __construct() {
		$this->mCacheKey = "methodguardian_standings2";
	}
	...
	function getTitle() {
		return wfMsg('methodguardian_leaderboard_title');
	}
}

class MethodEditorStandingsGroup extends StandingsGroup  {
    function __construct() {
        $this->mCacheKey = "methodeditor_standings2";
    }

    function getSQL($ts) {
        $sql = "SELECT user_name, count(*) as C ".
            "FROM logging left join wiki_shared.user on log_user = user_id ".
            "WHERE log_type = 'methedit' and log_timestamp >= '$ts' ".
            "GROUP BY user_name ORDER by C desc limit 25";
        return $sql;
    }
    ...
    function getTitle() {
        return wfMsg('methodeditor_leaderboard_title');
    }
}
