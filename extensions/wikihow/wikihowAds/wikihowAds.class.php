...
EOHTML;
	...
	function getAdUnit($num) {
		...
		if($wgLanguageCode == "en") {
			...
			if($tempTitle == "Get Caramel off Pots and Pans"){
				...
				$s = wfMsg('adunit' . $num, $channels[0], self::$hasAltMethods);
			}
		}
		else {
	...
	static public function getCSIParameters($adNum) {
		...
		$adSizes = array(
			"intro" =>		array("width" => 671, "height" => 60, "max_ads" => 1),
			"0" =>			array("width" => 629, "height" => 60, "max_ads" => 1),
	...
	function getCategoryAd() {
		...
		if($wgLanguageCode == "en") 
			$s = wfMsg('adunit-image-rightrail', $params[0], $params[1]);

		else
			$s = "<div class='side_ad'>" . wfMsg('adunit4') . "</div>";
	...
	function isEligibleForAds() {
		...
		if ($namespace == NS_SPECIAL && 
			($wgTitle->getText() == "Categorylisting" ||
			$wgTitle->getText() == "DocViewer" ||
			$wgTitle->getText() == "Quizzes")) {
			$isEligible = true;
		}
