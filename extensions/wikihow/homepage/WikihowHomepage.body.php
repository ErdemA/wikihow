...
class WikihowHomepage extends Article {
	...
	function view() {
		global $wgOut, $wgUser, $wgCategoryNames, $wgLanguageCode, $wgCategoryNamesEn;

		$wgHooks['ShowGrayContainer'][] = array('Misc::removeGrayContainerCallback');

		$faViewer = new FaViewer();
		$this->faStream = new WikihowArticleStream($faViewer, 0);
		...
		if($wgLanguageCode != "en") {
			$this->faStream = new WikihowArticleStream($faViewer, $this->faStream->getStreamPosition() + 1);
			...
			$rsViewer = new RsViewer();
			$this->rsStream = new WikihowArticleStream($rsViewer);
		...
		foreach($wgCategoryNames as $ck => $cat) {
			...
			if($wgLanguageCode != "en") {
				...
				$foreignCat = str_replace('-',' ',urldecode(wfMsg($msgKey)));
		...
		$sk->addWidget(wfMsg('main_page_worldwide_new', wfGetPad()));
